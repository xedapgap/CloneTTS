...
from .flow_matching import CausalConditionalCFM # Ensure this import is correct
...
class S3Token2Mel(torch.nn.Module):
    ...
    def __init__(self, cfg: Optional[DictConfig] = None):
        ...
        # Merge provided cfg with defaults
        self.cfm_params = default_cfm_params
        if cfg:
            self.cfm_params.update(cfg)

        decoder = CausalConditionalCFM(
            spk_emb_dim=80,
            cfm_params=self.cfm_params, # Pass the DictConfig here
            estimator=estimator,
        )
    ...
    # New method to set specific CFM inference parameters at runtime
    def set_inference_params(self, inference_cfg_rate: Optional[float] = None, sigma_min: Optional[float] = None):
        """Allows setting inference-related parameters after model initialization."""
        if inference_cfg_rate is not None:
            logging.info(f"Updating inference_cfg_rate to {inference_cfg_rate}")
            # CausalConditionalCFM directly exposes inference_cfg_rate
            self.flow.decoder.inference_cfg_rate = inference_cfg_rate 
        if sigma_min is not None:
            logging.info(f"Updating sigma_min to {sigma_min}")
            # Assuming sigma_min is *also* a direct attribute of CausalConditionalCFM/BASECFM
            self.flow.decoder.sigma_min = sigma_min # <-- CRUCIAL CHANGE HERE
    ...
    def forward(
        ...
        """
        assert (ref_wav is None) ^ (ref_dict is None), f"Must provide exactly one of ref_wav or ref_dict (got  and )"
...
# Keep this for the global S3Gen access point (already exists in your setup)
class S3Gen(S3Token2Wav):
    def __init__(self, cfg: Optional[DictConfig] = None):
        super().__init__(cfg=cfg)